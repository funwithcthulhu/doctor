  ...
  | Process.Spawn_error _ ->
      [
        Check.make ~id:"editor.vscode.command"
          ~title:"VS Code command not found (skipped)"
          Check.Ok;
      ]
  | Process.Exited 0 ->
      ...
      | _ ->
          [
            Check.make ~id:"editor.vscode.extensions"
              ...
              ~suggestion:
                "Open VS Code and check whether ocamllabs.ocaml-platform is installed."
              Check.Warn;
          ])
  | _ ->
      [
        Check.make ~id:"editor.vscode.command"
          ...
          ~suggestion:
            "Try running `code --version`, or reinstall the VS Code command-line launcher."
          Check.Warn;
      ]
